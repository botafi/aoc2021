...
var bestFuel = positions.foldl(a + abs(b - avgPoint), 0.0)
for step in 0..maxPosition:
    let up = positions.foldl(a + abs(b - (avgPoint + step.toFloat)), 0.0)
    let down = positions.foldl(a + abs(b - (avgPoint - step.toFloat)), 0.0)
